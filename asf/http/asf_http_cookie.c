...
PHP_METHOD(asf_http_cookie, __construct)
{
    ...
    add_assoc_long_ex(&sz, ASF_COOKIE_EXPIRE, ASF_COOKIE_EXPIRE_LEN, expire + 86400);

    if ((pzval = zend_hash_str_find(st, ASF_COOKIE_PATH, ASF_COOKIE_PATH_LEN)) && Z_TYPE_P(pzval) == IS_STRING) {
        add_assoc_stringl_ex(&sz, ASF_COOKIE_PATH, ASF_COOKIE_PATH_LEN, Z_STRVAL_P(pzval), Z_STRLEN_P(pzval));
    }
    ...
    if ((pzval = zend_hash_str_find(st, ASF_COOKIE_SECURE, ASF_COOKIE_SECURE_LEN)) && Z_TYPE_P(pzval) == IS_LONG) {
        add_assoc_long_ex(&sz, ASF_COOKIE_SECURE, ASF_COOKIE_SECURE, Z_LVAL_P(pzval));
    }
    
    if ((pzval = zend_hash_str_find(st, ASF_COOKIE_HTTPONLY, ASF_COOKIE_HTTPONLY_LEN))) {
        add_assoc_long_ex(&sz, ASF_COOKIE_HTTPONLY, ASF_COOKIE_HTTPONLY_LEN, Z_LVAL_P(pzval));
    }
}
/* }}} */
...
PHP_METHOD(asf_http_cookie, has)
{
    ...
    if (zend_hash_num_elements(cookie) < 1) {
        RETURN_FALSE;
    }
...
{
    zval *cookie = asf_http_req_pg_find(TRACK_VARS_COOKIE);
    size_t argc = zend_hash_num_elements(cookie);
    if (argc < 1) {
        RETURN_FALSE;
    }
    ...
    ZEND_HASH_FOREACH_STR_KEY(Z_ARRVAL_P(cookie), key) {
        if (key)  {
